  ...
  <div class="bitap-demo">
    ...
    <div class="step-view" v-if="step > 0 && currentStep">
      ...
      <div class="operation">
        ...
        <div class="op-arrow">↓ shift left, fill with 0</div>

        <div class="op-row">
          <span class="op-label">R &lt;&lt; 1</span>
          <span
            class="bit-cell"
            :class="[
              bit === 0 ? 'bit-zero' : 'bit-one',
              bit !== currentStep.prevR[i] || i === 0 ? 'bit-changed' : ''
            ]"
        ...
        <div class="op-arrow">↓ OR with mask for '{{ currentStep.textChar.toLowerCase() }}'</div>
        ...
        <div class="op-row">
          ...
          <span class="mask-hint">
            <template v-for="(ch, i) in patternChars" :key="i">
              <span :class="{ 'hint-match': currentStep.mask[i] === 0 }">{{ ch }}</span>
            </template>
          </span>
        </div>
      ...
      <div class="step-narrative">
        <template v-if="currentStep.isMatch">
          The last bit is <code>0</code> — the full pattern has been matched ending at position <strong>{{ step - 1 }}</strong>
          (match starts at <strong>{{ currentStep.matchPos }}</strong>).
        </template>
        <template v-else-if="currentStep.bits.some((b, i) => b === 0)">
          {{ countZeros(currentStep.bits) }} position{{ countZeros(currentStep.bits) > 1 ? 's' : '' }} still tracking
          ({{ describeZeros(currentStep.bits) }}) — partial match in progress.
        </template>
        <template v-else>
          All bits are <code>1</code> — no partial match is alive.
        </template>
      </div>
    </div>
    ...
    <div class="step-view initial" v-else-if="pattern && text">
      <div class="step-header">
        Initial state — all bits are <code>1</code> (no matches yet)
      </div>
      <div class="operation">
        <div class="op-row">
          <span class="op-label">R</span>
          <span class="bit-cell bit-one" v-for="i in patternLen" :key="'init' + i">1</span>
        </div>
      </div>
      <div class="position-labels">
        <span class="op-label"></span>
...
const examples = [
  ...
  { pattern: 'test', text: 'testing' },
  { pattern: 'ab', text: 'aab' },
]

const pattern = ref('abc')
const text = ref('xabcxy')
const step = ref(0)
...
const steps = computed<Step[]>(() => {
  ...
  const uniqueChars = new Set(t.split('').concat(p.split('')))
  for (const c of uniqueChars) {
    mask[c] = []
    for (let i = 0; i < m; i++) {
      mask[c].push(p[i] === c ? 0 : 1)
    }
  }
  const defaultMask = Array(m).fill(1)

  let R = Array(m).fill(1)

  for (let j = 0; j < t.length; j++) {
    const c = t[j]
    const charMask = mask[c] || defaultMask
    const prevR = [...R]
    const shifted = [0, ...R.slice(0, m - 1)]
    const newR = shifted.map((bit, i) => bit | charMask[i])
    const isMatch = newR[m - 1] === 0
...
function countZeros(bits: number[]): number {
  return bits.filter(b => b === 0).length
}

function describeZeros(bits: number[]): string {
  const positions = bits
    .map((b, i) => b === 0 ? `${patternChars.value[i]}₍${i}₎` : null)
    .filter(Boolean)
...
.dark .bit-cell.bit-zero { background: #1a3a2a; color: #75d49b; }
...
.dark .bit-cell.bit-one { background: #2a2a2a; color: #666; }
